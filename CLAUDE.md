...
**Next — Phase 5: media/player UI (STARTING — visual design already in hand).** Decided direction:
- **One JavaFX window per node.** `ServentMain` opens its own GUI; **keep a headless mode** (flag/arg) so `check.sh`, `MultipleServentStarter`, and scripted scenarios still run bezglavo. Node = its own GUI is faithful to real BitTorrent (1 node = 1 user's client; the localhost multi-process run simulates N separate machines, so N windows).
- **Introduce Gradle here** (to pull JavaFX + vlcj and drive `jpackage`/`jlink`). Gradle is build tooling only and does **NOT** relax the pure-JDK rule for `core.kademlia`/`core.transfer`. The UI only *reads from* the engine (peers/speeds/pieces/routing/RPC events) and *drives* `SlidingWindowPicker.setPlayhead`; add `DownloadSession.progress()` + engine event hooks to feed it.
- **Build incrementally:** app shell + control screens (Add Stream / Library / DHT Inspector) + the sliding-window piece strip first (immediately useful, no video), then embed **vlcj/libVLC** video + watch-while-downloading.
- **Visual spec is committed in `design_handoff_phase5_player/`** — HIGH-fidelity, recreate faithfully in FXML + JavaFX CSS. 5 screens (Now Playing, **Swarm map = the hero**, Library, Add Stream, DHT Inspector). Read `design_handoff_phase5_player/README.md` first; `DESIGN_TOKENS.md` + `theme.css` hold exact colors/type/spacing (Direction A "Midnight Neon"); `reference_screens/*.png` are the pixel reference. Fonts: Manrope + JetBrains Mono (bundle TTFs).

Before/with Phase 5, consider folding in the deferred engine/transfer gaps below when robustness matters.
