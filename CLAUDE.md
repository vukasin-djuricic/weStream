...
**Multi-peer swarming is now DONE (BEP 5 peer-set, 2026-06-16).** The DHT no longer holds a single seed per infohash: metadata lives under the `infohash` key (content-identical across sharers → overwrite-safe) and an accumulating, TTL'd **peer set** lives under a derived `peerSetKey(infohash)` (`PeerStore`, UNION-on-STORE + per-entry expiry, anti-spoofed by the wire source). `share` announces metadata + joins the swarm; `download`/`startDownload` resolve metadata, `getPeers`, and connect to **all** seeds in parallel (`DownloadSession` already swarms over N sockets); a downloader joins the swarm as a partial seed on its first verified piece and re-announces periodically. Verified by a two-seed `TransferCheck` (`peerCount==2`, byte-identical). Also hardened in the same pass: iterative `findValue`, RPC retry, LRU-bounded DHT store, k-bucket eviction wiring, and an API CSRF/path-confinement guard (see sections below). The frontend mock surfaces are gone — every screen is genuinely live.
...
**Java HTTP API — DONE (`src/app/api/`, pure JDK, OUTSIDE `core.*`).** `ApiServer` wraps the JDK's `com.sun.net.httpserver.HttpServer` (so `./check.sh`'s plain `javac src test` compiles it with no extra classpath — the engine's import rule stays intact); hand-rolled `Json` (writer + a flat-object reader for POST bodies). Owned/started by `NodeRuntime`; **API port = `11470 + (udpPort - 1100)`** (node 0 → 11470, node 1 → 11570). Cached thread pool (a long `/stream` response must not starve `/api/progress` polls), permissive **CORS + OPTIONS** at the single `requireMethod` chokepoint (the renderer is a different origin: Vite dev server / `file://` / the `<video>` tag). That chokepoint also enforces an **Origin CSRF guard** (a remote web page's cross-origin POST is rejected before it can act; no-Origin native/`<video>` requests and the renderer's localhost/`file://` origins pass). The `/api/download` `out` is **confined to the cache dir** (path-traversal/arbitrary-write rejected → 400). Endpoints, all covered by `test/app/api/ApiCheck` (real UDP+TCP+HTTP; `./check.sh` runs **78 API checks**) and verified live cross-process:
  - `GET /api/status` (nodeId/host/udp+api port/uptime/peerCount), `GET /api/routing` (bucketSizes[160] + contacts).
  - `POST /api/dht/put` `{key,value}` / `GET /api/dht/get?key=` → `storeValue`/`findValue` with the **same SHA-1 key derivation as the CLI** (interoperable). `GET /api/dht/keys` → local store snapshot (`storedCount`, `swarmCount`, key ids) for the inspector's Stored-keys panel.
  - `POST /api/download` now echoes resolved metadata (`pieceSize`/`totalLength`/`pieceCount`) so the UI's resolved card is real.
  - `POST /api/share` `{path}` → metadata+infohash; `POST /api/download` `{infohash[,out]}` → non-blocking `TransferService.startDownload` (uses `SlidingWindowPicker`, tracked in an `active` map; 404 if not announced). **Downloads are an ephemeral cache:** with no `out`, the default is `<user.dir>/cache/node-<udpPort>/<infohash>.bin` — a per-node scratch dir under the app folder, **wiped on startup + on JVM shutdown** (`TransferService.cleanCache()`, fired by a `NodeRuntime` shutdown hook so Electron's SIGTERM-kill cleans it). `GET /api/progress?infohash=` → live `{have,inFlight,total,peers,pieceStates[]}`; `GET /api/transfers` → the live Library list (`TransferService.transfers()` = seeded files + active downloads with progress).
  - `GET /api/rpclog` → recent RPC activity (newest-first, capped) for the inspector's live feed, from the engine's `RpcLog` ring (`KademliaService` logs every inbound/outbound RPC).
...
**Optional remaining:** the only non-functional work left is packaging — `electron-builder` (or `jpackage`/`jlink`) native bundle with a bundled JRE for the spawned Java. Also consider folding in the deferred engine/transfer gaps below when robustness matters.
...
**Engine hardening done (2026-06-16):** `findValue` is now a proper **iterative FIND_VALUE** (short-circuits on the first holder, follows NODES hops — no longer just delegating to `nodeLookup`); single-target RPCs (`ping`, STORE replication, peer announce) **retry once** via `awaitRetrying` to ride out UDP loss; the local DHT store is an **LRU-bounded `BoundedStore`** so the originator path no longer grows unbounded (the old `MAX_STORE_KEYS`-bypass / "Unbounded local store" gap is closed); and the k-bucket **eviction policy is now wired** — `onRaw` PINGs a full bucket's LRS candidate off-thread and replaces it only if dead. Note: the DHT `store` is for SMALL values (infohash → metadata) — actual piece/media bytes belong in the transfer layer's own cache, never the DHT map.

**Known engine gaps (still deferred):** serial awaits in `nodeLookup` (the ALPHA sends are parallel, but a dead node in a batch still costs one timeout before the round ends — overlapping rounds not implemented); no STORE TTL/republish (and `BoundedStore` deliberately has **no** TTL precisely because there's no republish — expiring a value with no refresh would drop still-wanted data); no `findValue` cache-on-read yet (safe to add now that the store is bounded). The peer-set `PeerStore` DOES have TTL + republish (re-announce), since peers are expected to refresh.

**Known transfer gaps (deferred).** **Multi-peer swarming is DONE** (BEP 5 peer-set — see the status section), so the old "single-seed only" gap is closed. Still deferred: no choking/unchoking or upload fairness (only matters with many peers); no endgame mode; `download` re-pulls the whole file (no resume/partial — `verifyAndMarkExisting` exists on the seed path but a restarted download doesn't reuse it yet); the full small metadata rides in the DHT value, which is fine for modest files but would need a metadata-vs-peerlist split for very large ones. A downloader DOES now re-announce itself as a partial seed (on first verified piece + periodic refresh). `download` uses `RarestFirstPicker`; `startDownload` (the streaming path) uses `SlidingWindowPicker` with `STREAM_WINDOW(32) > MAX_IN_FLIGHT(16)` so the near-playhead window keeps priority.
...
**Design-handoff references (committed, NOT code):** under `docs/design/` — `docs/design/design_handoff_phase5_player/` and `docs/design/phase5-2nd-iteration/design_handoff_phase5_player/` are the Phase-5 UI design specs — `DESIGN_TOKENS.md`, `theme.css`, `*.dc.html` mockups, and `reference_screens/*.png` (the 2nd-iteration set adds the sliding-window/table variants). They are the source-of-truth visual spec the live `react/westream-react/` UI was built against, kept for reference; they are not the running app and not engine code. The matching `*.zip` raw exports (e.g. `Faza 5 UI Dizajn.zip`) are gitignored as redundant binaries.
...
- `KademliaService` — the live node. Inbound RPCs handled on the transport receive thread (non-blocking: table lookup + reply); responses complete a pending `CompletableFuture`; timeouts enforced by a scheduler (no busy-wait, rule #4). `bootstrap(peer)`, iterative `nodeLookup(target)` (α=3 parallel rounds), `ping`, `storeValue`, `findValue`. **Blocking ops (`nodeLookup`/`ping`/`findValue`) must NOT be called from the receive thread** — they wait on futures completed by that thread, so doing so deadlocks. Hardened against hostile input: routing learns the **wire** source address (not the self-reported, spoofable endpoint), the codec rejects out-of-range type ordinals and over-long length fields (no OOM on bad packets), and `STORE` is capped (`MAX_STORE_KEYS`) against flooding. Covered by `./check.sh` (codec-fuzz, k-bucket eviction, an in-memory multi-hop chain lookup, a drop-transport timeout, and a real-UDP end-to-end group).
  - **Hardening done (2026-06-16):** `findValue` is now iterative (follows NODES, short-circuits on first holder); single-target RPCs retry once (`awaitRetrying`); the store is LRU-bounded (`BoundedStore`); k-bucket eviction is wired in `onRaw`. Plus BEP 5 multi-peer swarming (`PeerStore` + `announcePeer`/`getPeers`/`peerSetKey`). **Still deferred:** overlapping lookup rounds (a dead node in a batch still costs one timeout); STORE TTL/republish for the generic store; `findValue` cache-on-read.
